...
"undefined"==typeof usi_split_test&&(usi_split_test={split_test_name:"usi_dice_roll",split_group:"-1",split_siteID:"-1",split_test_cookie_length:3,get_split_var:function(t){if(t=t||"",null==usi_cookies.get("usi_visitor_id"+t)){var i=Math.random().toString(36).substring(2);i.length>6&&(i=i.substring(0,6));var s="v_"+i+"_"+Math.round((new Date).getTime()/1e3)+"_"+t;return usi_cookies.set("usi_visitor_id"+t,s,86400*this.split_test_cookie_length,!0),s}return usi_cookies.get("usi_visitor_id"+t)},report_test:function(t,i){var s=document.getElementsByTagName("head")[0],e=document.createElement("script");e.setAttribute("type","text/javascript"),e.setAttribute("src","https://www.upsellit.com/utility/split_test.jsp?siteID="+t+"&group="+i+"&usi_visitor_id="+this.get_split_var(t)),s.appendChild(e),void 0!==usi_split_test.set_verification&&setTimeout("usi_split_test.set_verification("+i+");",1e3)},get_group:function(t){return usi_cookies.get(this.split_test_name+t)},instantiate:function(t,i,s){null==usi_cookies.get(this.split_test_name+t)||s?(0===i||i&&""!=i?this.split_group=i:Math.random()>.5?this.split_group="0":this.split_group="1",this.report_test(t,this.split_group),usi_cookies.set(this.split_test_name+t,this.split_group,86400*this.split_test_cookie_length,!0)):this.split_group=usi_cookies.get(this.split_test_name+t)}});
		...
		usi_app.main = function () {
			...
			usi_app.recommendation_site = "31949";
			usi_app.product_page_data = {};
			usi_app.is_active_cart = document.querySelector('.cart_count') != null;

			// Pages
			usi_app.is_product_page = usi_app.url.path.contains("/products/");
			usi_app.is_category_page = usi_app.url.path.contains("/collections/");
			usi_app.is_cart_page = usi_app.url.path.full === "/cart";
			usi_app.is_checkout_page = usi_app.url.full.indexOf("/checkouts/") != -1;
			usi_app.is_confirmation_page = usi_app.url.path.full === "/CONFIRMATION";

			try {
				var usi_referrer = document.referrer.toLowerCase();
				usi_app.is_suppressed_perm = usi_app.url.full.toLowerCase().indexOf("shopstyle") !== -1 || usi_app.url.full.toLowerCase().indexOf("rewardstyle") !== -1 || usi_app.url.full.toLowerCase().indexOf("stylight") !== -1 || usi_referrer.indexOf("shopstyle") !== -1 || usi_referrer.indexOf("rewardstyle") !== -1 || usi_referrer.indexOf("stylight") !== -1 || usi_app.url.full.toLowerCase().indexOf("utm_source=135943") !== -1 || usi_app.url.full.toLowerCase().indexOf("utm_source=73861") !== -1 || usi_app.url.full.toLowerCase().indexOf("utm_source=20648") !== -1;
				if (usi_app.is_suppressed_perm) usi_cookies.set("usi_suppressed", "1", 31536000*1, true);
			} catch (err) {
				usi_commons.report_error(err);
			}
			...
			usi_app.is_suppressed = usi_app.confirmation || usi_cookies.value_exists("usi_suppressed");

			// Flags
			usi_app.force_group = usi_commons.gup_or_get_cookie('usi_force_group');

			// Collect product page datad
		...
		usi_app.load = function () {
			...
			if (usi_app.is_product_page || usi_app.is_category_page) {
					...
					usi_app.load_product_data({
						...
						callback: function(){
							if (typeof (usi_app.product_rec.product3) != "undefined") {
								usi_commons.log('[ load ] * * * Precise Promotion Split Test * * *');
								usi_commons.load_view("ZhtFYVJpuE5rdkyPZMFqMdX", "34863", usi_commons.device);
							}
						}
					});
				} else {
		...
		usi_app.scrape_product_page = function() {
			...
			try {
				var product = {};
				product.link = document.querySelector('meta[property="og:url"]').content;
				...
				product.price = document.querySelector('meta[property="og:price:amount"]').content;
				product.in_stock = usi_dom.get_first_element(".buy-button").textContent.trim() !== "Out Of Stock";
		...
		usi_app.send_product_data = function() {
			...
			usi_commons.send_prod_rec(usi_app.recommendation_site, product, true);
